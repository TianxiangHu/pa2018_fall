...
{//居然是可以使用modrm_r_rm()这个函数，出乎意料，比较，还是没能完全看懂这个函数的实现
	int len=1;
	...
	len+=modrm_r_rm(eip+1,&sr,&creg);
//	printf("c2r:after modrm_r_rm,crer.type:%x\n",creg.type);
//	assert(0);
	sr.type=OPR_REG;
	creg.type=OPR_CREG;
	operand_read(&creg);
...
{
	int len = 2;
	uint32_t cur = paddr_read(cpu.eip+1,1);
	uint32_t r = cur & 0x7;//寄存器
	uint32_t sr = (cur >> 3) & 0x7;//段寄存器
	
	cpu.segReg[sr].val = cpu.gpr[r].val;
