...
int fs_open(const char *pathname, int flags) {
	...
	{
		if((strcmp(pathname,file_table[i].name)==0) && (files[i+3].used==false))
		{
...
size_t fs_read(int fd, void *buf, size_t len) {
	...
	uint32_t size_cur=0;
	if((files[fd].offset+len)<(file_table[fd-3].size+file_table[fd-3].disk_offset))
		size_cur=len;
	else
		size_cur=file_table[fd-3].size+file_table[fd-3].disk_offset-files[fd].offset;
	if(size_cur==0)
		;
	else
	{
		ide_read(buf,files[fd].offset,size_cur);
		files[fd].offset+=len;
	}
	return size_cur;
}
